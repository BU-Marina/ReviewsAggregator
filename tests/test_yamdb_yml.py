...
class TestWorkflow:
    ...
    def test_workflow(self):
        workflow_basename = 'ci'

        yaml = f'{workflow_basename}.yaml'
        is_yaml = yaml in os.listdir(root_dir)

        yml = f'{workflow_basename}.yml'
        ...
        if is_yaml and is_yml:
            assert False, (
                f'В каталоге {root_dir} не должно быть двух файлов {workflow_basename} '
        ...
        assert (
                re.search(r'on:\s*push:\s*branches:\s*-\smaster', yml_file) or
                'on: [push]' in yml_file or
                'on: push' in yml_file
        ), f'Проверьте, что добавили действие при пуше в файл {filename}'
        assert 'pytest' in yml_file, f'Проверьте, что добавили pytest в файл {filename}'
        assert 'appleboy/ssh-action' in yml_file, f'Проверьте, что добавили деплой в файл {filename}'
        assert 'appleboy/telegram-action' in yml_file, (
